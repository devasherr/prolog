...
func (l *DistributedLog) setupRaft(dataDir string) error {
	...
	// key-value store where raft stores metadata
	...
	retain := 1
	snapStore, err := raft.NewFileSnapshotStore(filepath.Join(dataDir, "raft"), retain, os.Stderr)
	if err != nil {
		return err
	}

	maxPool := 5
	transport := raft.NewNetworkTransport(l.config.Raft.StreamLayer, maxPool, 10*time.Second, os.Stderr)
