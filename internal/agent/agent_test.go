...
import (
	...
	"github.com/devasherr/prolog/internal/agent"
...
func TestAgent(t *testing.T) {
	...
	time.Sleep(3 * time.Second) // time for node to discover eachother
	...
	require.NoError(t, err)
	consumeResponse, err := leaderClient.Consume(ctx, &api.ConsumeRequest{
		Offset: produceResponse.Offset,
	})
	require.NoError(t, err)
	require.Equal(t, consumeResponse.Record.Value, []byte("foo"))
	time.Sleep(3 * time.Second) // time for replication

	followerClient := client(t, agents[1], peerTLSConfig)
	consumeResponse, err = followerClient.Consume(ctx, &api.ConsumeRequest{
		Offset: produceResponse.Offset,
	})
	require.NoError(t, err)
	require.Equal(t, consumeResponse.Record.Value, []byte("foo"))
